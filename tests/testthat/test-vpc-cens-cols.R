...
test_that(".vpcCensCol errors informatively on missing and ambiguous columns", {
  ...
  expect_error(.vpcCensCol(.d, "tad", "observed"), "cannot find a unique 'tad' column in the observed data")

  # ambiguous case-insensitive match: no exact "time", but two candidates
  .amb <- data.frame(ID = 1, TIME = 1, Time = 2, DV = 3)
  expect_error(.vpcCensCol(.amb, "time", "simulated"), "matched: TIME, Time")
})
...
test_that(".vpcCensDropStray drops only unmapped id/dv/idv columns", {
  ...
  expect_equal(names(.vpcCensDropStray(.d2, .cols2)), c("id", "sim", "time"))
...
test_that(".vpcCensObs errors on an ambiguous cens column (#55)", {
  ...
  expect_error(.vpcCensObs(.d), "cannot find a unique 'cens' column in the observed data")
})
...
test_that("vpc_cens counts each censored record only on its own side (#55)", {
  ...
  .frac <- function(lloq = NULL, uloq = NULL) {
    .db <- suppressWarnings(suppressMessages(vpc::vpc_cens(
      sim = .sim,
...
test_that(".vpcSimData needs a CMT column for a non-normal endpoint (#68)", {
  ...
  .fit <- list(
    ui = list(predDf = data.frame(distribution = c("norm", "pois"), cmt = c(3L, 4L))),
  ...
  # is turned off
  .d$CMT <- NULL
  expect_false(grepl("needs a 'CMT' column", .msg2(.fit, .d, normRelated = FALSE)))
